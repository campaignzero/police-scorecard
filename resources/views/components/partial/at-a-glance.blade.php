...
$class_h = (is_numeric($scorecard['report']['percent_murders_solved'])) ? 'key animate-score percent-' . round($scorecard['report']['percent_murders_solved']) : 'incomplete';
$class_i = (is_numeric($scorecard['report']['percentile_jail_incarceration_per_1k_population'])) ? 'key animate-score percent-' . round($scorecard['report']['percentile_jail_incarceration_per_1k_population']) : 'incomplete';
$class_j = (is_numeric($scorecard['report']['percentile_jail_deaths_per_1k_jail_population'])) ? 'key animate-score percent-' . round($scorecard['report']['percentile_jail_deaths_per_1k_jail_population']) : 'incomplete';
$class_k = (is_numeric($scorecard['report']['percentile_police_spending'])) ? 'key animate-score percent-' . round($scorecard['report']['percentile_police_spending']) : 'incomplete';
$class_l = (is_numeric($scorecard['police_funding']['percentile_misconduct_settlements_per_population'])) ? 'key animate-score percent-' . round($scorecard['police_funding']['percentile_misconduct_settlements_per_population']) : 'incomplete';
$class_m = (is_numeric($scorecard['police_funding']['percentile_fines_forfeitures_per_resident'])) ? 'key animate-score percent-' . round($scorecard['police_funding']['percentile_fines_forfeitures_per_resident']) : 'incomplete';
$class_n = (is_numeric($scorecard['police_funding']['percentile_officers_per_population'])) ? 'key animate-score percent-' . round($scorecard['police_funding']['percentile_officers_per_population']) : 'incomplete';
$class_o = (is_numeric($scorecard['report']['percent_use_of_force_complaints_sustained'])) ? 'key animate-score percent-' . round($scorecard['report']['percent_use_of_force_complaints_sustained']) : 'incomplete';
$class_p = (is_numeric($scorecard['report']['percent_discrimination_complaints_sustained'])) ? 'key animate-score percent-' . round($scorecard['report']['percent_discrimination_complaints_sustained']) : 'incomplete';
$class_q = (is_numeric($scorecard['report']['percentile_drug_arrest_disparity'])) ? 'key animate-score percent-' . round($scorecard['report']['percentile_drug_arrest_disparity']) : 'incomplete';
@endphp
...
<div class="section bg-gray at-a-glance">
    <div class="content group-header">
        ...
        <p>Scores range from 0-100% comparing {{ $type === 'sheriff' ? 'counties' : 'cities' }} with {{ $scorecard['police_funding']['comparison_group'] }} population. {{ $type === 'sheriff' ? 'Counties' : 'Cities' }} with higher scores spend less on policing, use less force, are more likely to hold officers accountable and make fewer arrests for low-level offenses.</p>
    </div>
